...
C      REAL :: equivStress, fractureWorkInc, smean, stressPower, two
      ...
      REAL(KIND=8), DIMENSION(1:2)::TIME
     ...
     $     sM_fin, deL, deM, eps_N0_neg, eps_N0_pos
      ...
      REAL(KIND=8) , DIMENSION(1:NTENS) :: eps, deps, sig_old 
      REAL(KIND=8) , DIMENSION(1:NTENS) :: sig
      REAL(KIND=8) , DIMENSION(1:NTENS,1:NTENS) :: jacobian
...
c      eps = 0.0d0
c      deps = 0.0d0
c      sig_old = [STRESS(1), STRESS(2), 0.D0,STRESS(3), 0.D0,0.D0]*unit_conv
c      write(6,*), "STRESS_OLD=",STRESS
c      write(6,*), "MSTRAN=",STRAN
c      eps = [STRAN(1), STRAN(2), 0.D0 ,STRAN(3), 0.D0, 0.D0]
c      write(6,*), "Meps=",eps
c      deps = [DSTRAN(1), DSTRAN(2), 0.D0, DSTRAN(3), 0.D0, 0.D0]
c      write(6,*), "Mdeps=",deps
      DO IC = 1, NTENS
          sig_old(IC) = STRESS(IC)*unit_conv
          eps(IC) = STRAN(IC)
          deps(IC) = DSTRAN(IC)
      END DO      
